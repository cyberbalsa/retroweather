...
signing_key_pem="$SIGNED_DIR/weatherstarkiosk-signing-key.pem"
python3 "$FIDO2_SCRIPT" export-pubkey > "$signing_key_pem"
...
[[ -f "$signing_key_pem" ]] && drop_asset "$RELEASE_TAG" "$(basename "$signing_key_pem")"
...
[[ -f "$signing_key_pem" ]] && gh release upload "$RELEASE_TAG" "$signing_key_pem" --repo "$REPO"
